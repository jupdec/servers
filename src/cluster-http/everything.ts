...
import { listClusters, describeCluster, listClustersSchema, describeClusterSchema } from "./cluster-tools.js";
...
enum ToolName {
  ...
  DESCRIBE_CLUSTER = "describe-clusters",
}
...
export const createServer = () => {
    ...
    const tools: Tool[] = [
      {
        ...
        inputSchema: zodToJsonSchema(listClustersSchema) as ToolInput,
      },
      {
        name: ToolName.DESCRIBE_CLUSTER,
        description: "Get detailed information about a specific cluster",
        inputSchema: zodToJsonSchema(describeClusterSchema) as ToolInput,
      },
    ];
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === ToolName.LIST_CLUSTERS) {
      listClustersSchema.parse(args);
    ...
    if (name === ToolName.DESCRIBE_CLUSTER) {
      const validatedArgs = describeClusterSchema.parse(args);
      try {
        const cluster = describeCluster(validatedArgs);
        return {
          content: [
            {
              type: "text",
        ...
        throw new Error(`Error describing cluster: ${(error as Error).message}`);
      }
    }
